...
<script>
import { setCardDoctorNum } from "@/api/getanalysisdata";
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      setCardDoctorNum(data).then(response => {
    ...
    exportData() {
      ...
      let startTime = this.$store.state.dataanalysis.searchTime[0];
      let endTime = this.$store.state.dataanalysis.searchTime[1];
      exportData('exportBuildCardDoctors?startTime='+startTime+'&endTime='+endTime);
    }
  },
